-- ============================================
-- 云文件系统 - 目录树系统数据库脚本 v3.0 (乐观锁版)
-- 创建日期: 2026-06-03
-- 说明: 支持回收站、待分配目录池、软删除机制、乐观锁并发控制
-- 变更: 新增 version 字段用于乐观锁，防止并发冲突
-- ============================================
...
CREATE TABLE `file_metadata` (
    ...
    -- 乐观锁版本号（可选，用于引用计数保护）
    `version` BIGINT DEFAULT 0 COMMENT '乐观锁版本号（可选）',
...
-- 执行完成提示
-- ============================================
SELECT '数据库脚本 v3.0（乐观锁版）执行完成！' AS message;
...
-- 验证乐观锁字段
SELECT 'folder_nodes version field:' AS info;
SHOW COLUMNS FROM folder_nodes LIKE 'version';

SELECT 'file_nodes version field:' AS info;
